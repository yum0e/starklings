...
func square(array : felt*, array_len : felt, new_squared_array : felt*):
    if array_len == 0:
        return ()
    end

    let squared_item = array[0] * array[0]
    assert [new_squared_array] = squared_item

    return square(array + 1, array_len - 1, new_squared_array + 1)
end
...
func test_square{syscall_ptr : felt*}():
    ...
    let (new_squared_array : felt*) = alloc()
    ...
    square(array, 4, new_squared_array)

    assert [new_squared_array] = 1
    assert [new_squared_array + 1] = 4
    assert [new_squared_array + 2] = 9
    assert [new_squared_array + 3] = 16
